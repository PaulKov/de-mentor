...
- `greenplum-theory.pptx` - светлая русскоязычная презентация для первого урока: 24 слайда основного маршрута и 8 appendix/deep-route слайдов.
...
- `partitioning-strategies.md` - deep-dive по `PARTITION BY RANGE`, `PARTITION BY LIST`, `PARTITION BY HASH`, `DEFAULT partition`, `pg_partition_tree`, `gp_toolkit.gp_partitions` и `leaf_partitions`.
