# Гайд Ведущего: Теоретическая Презентация Greenplum
...
- упрощенный маршрут: 60 минут, слайды 1-24, короткая практика и домашка.
- расширенный маршрут: 90-120 минут, упрощенный маршрут плюс appendix 25-32.

Связанные материалы:

- [Упрощенный маршрут](https://github.com/PaulKov/de-mentor/blob/master/lessons/lesson-01/docs/runbooks/simple-path.md)
- [Расширенный маршрут](https://github.com/PaulKov/de-mentor/blob/master/lessons/lesson-01/docs/runbooks/deep-dive-path.md)
- [План домашки](https://github.com/PaulKov/de-mentor/blob/master/lessons/lesson-01/homework/plan.md)
- [Workbook ученика](https://github.com/PaulKov/de-mentor/blob/master/lessons/lesson-01/docs/student-workbook.md)
- [QD/QE/gang/slices explained](https://github.com/PaulKov/de-mentor/blob/master/lessons/lesson-01/docs/deep-dives/qd-qe-gang-slices-explained.md)
- [Cluster inspection SQL](https://github.com/PaulKov/de-mentor/blob/master/labs/greenplum/examples/cluster-inspection.sql)
- [Cluster monitoring SQL](https://github.com/PaulKov/de-mentor/blob/master/labs/greenplum/examples/cluster-monitoring.sql)
- [Storage and partitioning SQL](https://github.com/PaulKov/de-mentor/blob/master/labs/greenplum/examples/storage-and-partitioning.sql)
...
| 12:00-15:00 | 6. QD / QE / gang / slices | 3 | Простая аналогия: QD - диспетчер, QE - исполнители, gang - команда, slice - участок работы. | Что такое gang простыми словами? | Для цельного объяснения открыть [QD/QE/gang/slices explained](https://github.com/PaulKov/de-mentor/blob/master/lessons/lesson-01/docs/deep-dives/qd-qe-gang-slices-explained.md). |
| 15:00-18:00 | 7. QD/QE dispatch deep dive | 3 | QD режет plan на slices, выделяет gang, создает QueryDispatchDesc, QE исполняют slice. | Почему slice часто рядом с Motion? | Углубление только по готовности ученика; затем [master/segment data path](https://github.com/PaulKov/de-mentor/blob/master/lessons/lesson-01/docs/deep-dives/master-segment-data-path.md). |
...
| 25:00-28:00 | 11. Docker lab cluster passport | 3 | У нас один Docker service `greenplum`: 1 coordinator/master, 2 primary segments, 0 mirrors, 1 segment host. CPU/RAM limits не заданы в compose. | Где проверить это через SQL? | Выполнить [cluster inspection SQL](https://github.com/PaulKov/de-mentor/blob/master/labs/greenplum/examples/cluster-inspection.sql). |
| 28:00-31:00 | 12. Heap vs AOCO DDL | 3 | Покажи `appendoptimized=true`, `orientation=column`, `\d+` и catalog checks. | Что проверяет `access_method`? | Выполнить [storage and partitioning SQL](https://github.com/PaulKov/de-mentor/blob/master/labs/greenplum/examples/storage-and-partitioning.sql). |
...
| 57:00-60:00 | 24. Что ученик должен унести | 3 | Greenplum = MPP; master control plane; segments data plane; skew измерим; Motion виден; фиксы доказываются. | Один принцип, который применишь в следующей модели? | Выдать [homework](https://github.com/PaulKov/de-mentor/blob/master/lessons/lesson-01/homework/assignment.md) и Lesson 02. |

## Расширенный Маршрут: Appendix 25-32
...
| --- | ---: | --- | --- | --- |
| 25. APPENDIX: как читать EXPLAIN | 6 | Ученик видит Motion, но теряется в плане. | План читаем как маршрут: scans -> local work -> join -> Motion -> global work -> Rows out. | Заполнить ladder в [student workbook](https://github.com/PaulKov/de-mentor/blob/master/lessons/lesson-01/docs/student-workbook.md). |
...
| 29. QD/QE internals with source anchors | 6 | Нужна глубина по QD/QE без чтения всего C-кода. | Якоря исходного кода: `cdbdisp_query.c`, `nodeMotion.c`, `nodeHashjoin.c`, `joinpath.c`, `CdbPathLocus`. | Прочитать [QD/QE/gang/slices explained](https://github.com/PaulKov/de-mentor/blob/master/lessons/lesson-01/docs/deep-dives/qd-qe-gang-slices-explained.md), затем связать slice/gang/Motion с планом. |
...
| 32. Partition catalog: как смотреть и считать | 6 | Ученик спрашивает, как увидеть partitions и сколько их в GP. | `pg_partition_tree` показывает hierarchy; `gp_toolkit.gp_partitions` дает inventory leaf partitions; `leaf_partitions` считаем через `COUNT(*) FILTER (WHERE isleaf)`. | Запустить [partitioning strategies SQL](https://github.com/PaulKov/de-mentor/blob/master/labs/greenplum/examples/partitioning-strategies.sql), затем обсудить `ATTACH PARTITION`, `DETACH PARTITION` и out-of-range INSERT. |
...
- Если время проседает, сократи appendix до ссылок на [расширенный маршрут](https://github.com/PaulKov/de-mentor/blob/master/lessons/lesson-01/docs/runbooks/deep-dive-path.md), но не выкидывай практику и homework handoff.
