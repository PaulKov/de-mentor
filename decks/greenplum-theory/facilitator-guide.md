...
- deep-dive path: 90-120 минут, simple path плюс appendix 25-31.
...
| 40:00-42:00 | 17. Partitioning intro | 2 | partitioning intro: pruning/retention != distribution. `PARTITION BY RANGE (sale_date)` помогает датам, но не размещает строки по сегментам. | Почему partition key не равен distribution key? | Глубина переносится в Lesson 02. |
...
- На слайде 17 явно скажи: partitioning intro сегодня, deep partitioning lab на Lesson 02.
