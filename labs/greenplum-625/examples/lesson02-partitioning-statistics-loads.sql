-- Greenplum 6.25 lesson 02: partitioning, statistics and incremental loads.
...
--
-- GP6: classic PARTITION BY ... (START/END), appendonly=true.
-- Catalog: pg_partitions (pg_partition_tree is GP7+/PG10 declarative).
...
(
    ...
    END (TIMESTAMP '2026-02-01') EXCLUSIVE,
    START (TIMESTAMP '2026-02-01') INCLUSIVE
    END (TIMESTAMP '2026-03-01') EXCLUSIVE
);
...
\echo '07. Partition catalog checks: pg_partitions (GP6)'
SELECT
    schemaname,
    tablename,
    partitiontablename,
...
\echo '10. Maintenance snippets (GP6 classic partitions)'
-- Add next month:
...
--
-- Load contract: stage → validate → publish → ANALYZE → audit.
